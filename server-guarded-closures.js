...
function patchRoute(method, original) {
  express.application[method] = function patchedRoute(path, ...handlers) {
    if (String(path).startsWith('/api/')) installGuardedRoutes(this)
...
express.application.listen = function guardedListen(...args) {
  ...
  console.log(`Scheduled transitions and day closures use ${config.timeZone} with server-authoritative reconciliation on port ${config.port}.`)
  return server
}
