export function laborShareSlackPlugin() {
  return {
    name: 'staffboard-labor-share-slack',
    enforce: 'pre',
    transform(code, id) {
    ...
    if (type === 'tph') return [scopeLine, 'Live SPEED TPH: ' + currentLiveTPH.toFixed(1) + ' | Required SPEED TPH: ' + metrics.requiredTPH.toFixed(1), 'Goal completion: ' + efficiencyPct.toFixed(0) + '% | Risk: ' + risk, ...laborLines, generatedLine].join(String.fromCharCode(10))
    if (type === 'issues') return [scopeLine + ' — Staffing Issues', issues.length ? issues.join(' | ') : 'No major staffing issues detected.', ...laborLines, generatedLine].join(String.fromCharCode(10))
    return [scopeLine, ...laborLines, 'Live SPEED TPH: ' + currentLiveTPH.toFixed(1), 'Required SPEED TPH: ' + metrics.requiredTPH.toFixed(1), 'Risk: ' + risk, 'Issues: ' + (issues.length ? issues.join(', ') : 'none'), generatedLine].join(String.fromCharCode(10))
  }
