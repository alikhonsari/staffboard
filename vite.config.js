...
export default defineConfig({
  plugins: [weekIsolationPlugin(), clearDayPlugin(), enhancementSuitePlugin(), shiftScopeCorePlugin(), shiftOpsLogicPlugin(), shiftOpsSyntaxFixPlugin(), enhancementNavPlugin(), managerTabPlugin(), auditTabPlugin(), toolsTabPlugin(), suggestionsTabPlugin(), shiftContextUxPlugin(), analysisScopePlugin(), auditAdminPlugin(), auditActionsPlugin(), shiftCorePlugin(), reportExportPlugin(), reportShiftFixPlugin(), pdfDailyDataPlugin(), pdfWeeklyDataPlugin(), pdfDailySectionPlugin(), pdfWeeklySectionPlugin(), pdfFooterPlugin(), builderTagPlugin(), scheduledStatusPlugin(), react()],
  server: {
    host: '0.0.0.0',
    port: 5173,
