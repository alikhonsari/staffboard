...
export default defineConfig({
  plugins: [weekIsolationPlugin(), clearDayPlugin(), enhancementSuitePlugin(), resetWeekScopePlugin(), shiftScopeCorePlugin(), laborShareCorePlugin(), weekMetadataScopePlugin(), shiftOpsLogicPlugin(), shiftOpsSyntaxFixPlugin(), laborShareComparisonPlugin(), laborShareLogicPlugin(), crossBoardAuditPlugin(), exportContextPlugin(), excelScopePlugin(), enhancementNavPlugin(), auditTabPlugin(), toolsTabPlugin(), shiftContextUxPlugin(), analysisScopePlugin(), laborShareAreaAdminPlugin(), laborShareMovementPlugin(), laborShareBoardUiPlugin(), laborShareAnalysisPlugin(), laborShareSuggestionsPlugin(), laborShareSlackPlugin(), managerTabPlugin(), suggestionsTabPlugin(), auditAdminPlugin(), auditActionsPlugin(), shiftCorePlugin(), reportExportPlugin(), reportShiftFixPlugin(), laborShareExcelPlugin(), pdfDailyDataPlugin(), pdfWeeklyDataPlugin(), pdfDailySectionPlugin(), pdfWeeklySectionPlugin(), pdfFooterPlugin(), laborSharePdfPlugin(), builderTagPlugin(), scheduledStatusPlugin(), react()],
  server: {
    host: '0.0.0.0',
    port: 5173,
