...
test('startup hydration transform runs before App save hardening', () => {
  assert.match(viteSource, /startupHydrationPlugin/)
  assert.match(viteSource, /requestTimeoutPlugin\(\), startupHydrationPlugin\(\), postgresStateSavePlugin\(\)/)
})
